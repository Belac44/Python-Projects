...
def countdown(count):
    ...
    if count > 0:
        ...
        start_timer()
        stars = "/" * math.floor(reps / 2)
        check.config(text=stars)
# ---------------------------- UI SETUP ------------------------------- #
window = Tk()


window.title("Pomodoro Time Management System")
window.config(padx=100, pady=50, bg=YELLOW)

label_text = Label(text="Timer", bg=YELLOW, fg=GREEN, font=(FONT_NAME, 50))
